...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    connectSocket(() => { fetchSlots(); fetchMyBookings(); fetchQueueInfo(); });
  ...
  const bookSlot = async (overrideSlot) => {
    ...
    try {
      await API.post(`/bookings/book?slotNumber=${target}`);
      setSlotNumber(""); fetchSlots(); fetchMyBookings();
  ...
  const handleConfirm = async () => {
    ...
    setConfirm(null);
    if (type === "release") {
      try { await API.put(`/bookings/release/${bookingId}`); fetchSlots(); fetchMyBookings(); toast("Slot released.", "info"); }
  ...
  return (
    <div className="min-h-screen bg-[#f6efe5] text-neutral-900 flex flex-col">
      ...
      {/* Top Header */}
      <header className="sticky top-0 z-50 bg-white/70 backdrop-blur-md border-b border-black/5 w-full">
        <div className="flex items-center justify-between px-6 py-3 w-full">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden sm:flex items-center gap-2 rounded-full bg-white border border-black/10 px-3 py-1.5">
              <div className="h-6 w-6 rounded-full bg-neutral-950 text-white text-xs font-bold flex items-center justify-center">U</div>
              <span className="text-xs font-semibold text-neutral-700">My Account</span>
            </div>
            <button onClick={logout} className="rounded-full border border-black/10 bg-white px-4 py-1.5 text-sm font-semibold hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition">
              Log out
            </button>
          </div>
        </div>
      ...
      <div className="flex flex-1">
        ...
        {/* Sidebar */}
        <aside className={`fixed md:sticky top-[53px] h-[calc(100vh-53px)] z-40 w-56 bg-white border-r border-black/5 flex flex-col py-6 px-3 transition-transform duration-200
        ...
        <main className="flex-1 px-6 py-8 max-w-5xl mx-auto w-full flex flex-col gap-8">
          ...
          {activeNav === "overview" && (
            <>
              <div>
                <h1 className="text-2xl font-bold tracking-tight text-neutral-950">Good day 👋</h1>
                ...
                {loading ? (
                  <table className="w-full text-sm">
                    <thead><tr className="bg-neutral-50 border-b border-black/5 text-left">
                      {["ID","Slot","Status","Cost","Actions"].map(h => <th key={h} className="px-6 py-3 text-xs font-semibold text-neutral-400 uppercase tracking-wider">{h}</th>)}
                    </tr></thead>
                  ...
                  <div className="flex flex-col items-center justify-center py-16 gap-3">
                    ...
                    <p className="text-sm font-semibold text-neutral-500">No bookings yet</p>
                    <button onClick={() => setActiveNav("slots")} className="text-xs text-neutral-950 underline">Browse slots →</button>
                  </div>
                ) : (
                        ...
                        {myBookings.map(b => (
                          <tr key={b.id} className="hover:bg-neutral-50/60 transition">
                            ...
                            <td className="px-6 py-4 font-semibold">
                              {b.active ? <span className="text-neutral-400 text-xs">Pending</span> : <span className="text-emerald-700">₹{b.cost?.toFixed(0) ?? "—"}</span>}
                            </td>
                            <td className="px-6 py-4">
                              {b.active && (
